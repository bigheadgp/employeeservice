...
import com.example.employee.dao.mapper.EmployeeMapper;
...
public class EmployeeMapperIntegrationTest {
    ...
    public void whenBirthOk_thenReturn2Employees() {
        ...
        saveAllAndFlush(allEmployees);

        List<Employee> searchResults = employeeMapper.selectEmployeeByBirthdateBetween(from, to);
        ...
        deleteAll(allEmployees);
    }

    private void saveAllAndFlush(Iterable<Employee> employees){
        for (Employee employee: employees) {
            entityManager.persistAndFlush(employee);
        }
    }

    private void deleteAll(Iterable<Employee> employees) {
        for (Employee employee: employees) {
